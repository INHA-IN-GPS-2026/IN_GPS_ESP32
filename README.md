...
> **현재 브랜치 `feature/ulp-30min-ble-logger`**: 30분 동안 무선 없이 1초 기록 1,800개를 수집하고(5분 단축 검증 가능) nRF Connect/PC에 직접 BLE GATT로 전송합니다. ULP + RTC 이중 버퍼 + 전용 Flash 저장, 빌드/사용법은 [LOGGER_README.md](LOGGER_README.md)를 참조하세요. 아래 rev 4.0 설명은 보존된 기존 게이트웨이 광고 애플리케이션(`CONFIG_INGPS_BATCH_LOGGER=n`)에 관한 내용입니다.
